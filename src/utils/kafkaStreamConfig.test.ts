...
describe("buildKafkaStreamConfigMap", () => {
  ...
  it("respects consumer type and offset reset", () => {
    const m = buildKafkaStreamConfigMap({
      ...base,
      consumeType: "highlevel",
      autoOffsetReset: "largest",
    });
    expect(m["stream.kafka.consumer.type"]).toBe("highlevel");
    expect(m["stream.kafka.consumer.prop.auto.offset.reset"]).toBe("largest");
  });

  it("emits SASL and security props with bare Kafka keys", () => {
    const m = buildKafkaStreamConfigMap({
      ...base,
    ...
    expect(m["sasl.mechanism"]).toBe("SCRAM-SHA-256");
    expect(m["security.protocol"]).toBe("SASL_SSL");
    expect(m["sasl.jaas.config"]).toContain("ScramLoginModule");
  });

  it("emits custom consumer properties with keys as entered", () => {
    const m = buildKafkaStreamConfigMap({
      ...base,
      consumerExtraProps: { "max.poll.records": "500" },
    });
    expect(m["max.poll.records"]).toBe("500");
  });

  it("allows full stream.* keys in consumer extra", () => {
    const m = buildKafkaStreamConfigMap({
      ...base,
  ...
  it("emits segment flush keys when provided", () => {
    ...
    expect(m["realtime.segment.flush.threshold.size"]).toBe("150M");
    expect(m["realtime.segment.flush.threshold.time"]).toBe("24h");
    expect(m["realtime.segment.flush.threshold.autotune.initialRows"]).toBe("50");
  });
});
